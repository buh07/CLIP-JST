...
run_step "stage5_e8a_gpu7_flickr_s0" "python -m MultiModal.multimodal.experiments.run_stage5_e8a_concat --config MultiModal/configs/stage5_e8a_gpu7_flickr_s0.yaml"
...
if [[ "$RUN_STAGE6" == "true" ]]; then
  run_step "stage6_e8b_gpu7_flickr_s0" "python -m MultiModal.multimodal.experiments.run_stage6_e8b_mask_concat --config MultiModal/configs/stage6_e8b_gpu7_flickr_s0.yaml"
fi
...
if [[ "$RUN_STAGE8" == "true" ]]; then
  ...
  run_step "stage8_e8d_gpu7_eps14" "python -m MultiModal.multimodal.experiments.run_stage8_e8d_dpsgd --config MultiModal/configs/stage8_e8d_dpsgd_gpu7_eps14.yaml"
fi

echo "[$(date '+%F %T')] GPU7 split queue complete."
