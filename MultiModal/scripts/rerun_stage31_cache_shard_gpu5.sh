...
python MultiModal/scripts/build_wavcaps_cache_shard.py \
  --shard-out-dir "$SHARD_ROOT/shard1" \
  ...
  --shard-index 1 \
  --shard-count 4 \
  --done-marker "$MARK_DIR/wavcaps_cache.shard1.done.json" \
  --fail-marker "$MARK_DIR/wavcaps_cache.shard1.fail.json" \
  2>&1 | tee "$LOG_DIR/stage31_cache_shard_gpu5.log"
