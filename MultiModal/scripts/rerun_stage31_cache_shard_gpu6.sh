...
python MultiModal/scripts/build_wavcaps_cache_shard.py \
  --shard-out-dir "$SHARD_ROOT/shard2" \
  ...
  --shard-index 2 \
  --shard-count 4 \
  --done-marker "$MARK_DIR/wavcaps_cache.shard2.done.json" \
  --fail-marker "$MARK_DIR/wavcaps_cache.shard2.fail.json" \
  2>&1 | tee "$LOG_DIR/stage31_cache_shard_gpu6.log"
