...
if [[ -f "$MARK_DIR/vitl14_stage44.ready.fail" ]]; then
  echo "[$(date '+%F %T')] stage39 gpu7 aborting: vitl14 stage44 failed"
  exit 1
fi
...
python -m MultiModal.multimodal.experiments.run_stage39_modality_gap_linear_vs_jl \
  --config MultiModal/configs/stage39_vitl14_gpu7.yaml 2>&1 | tee "$LOG_DIR/stage39_vitl14_gpu7.log"

touch "$MARK_DIR/gpu7.vitl14_stage39.done"
echo "[$(date '+%F %T')] queue_postw3_vitl14_stage39_gpu7 complete"
