...
python -m MultiModal.multimodal.experiments.run_stage48_sdpi_embed_export \
  --config MultiModal/configs/stage48_sdpi_gpu4.yaml 2>&1 | tee "$LOG_DIR/stage48_gpu4.log"
touch "$MARK/gpu4.stage48.done"
...
python -m MultiModal.multimodal.experiments.run_stage49_sdpi_mi_diagnostics \
  --config MultiModal/configs/stage49_sdpi_gpu4.yaml 2>&1 | tee "$LOG_DIR/stage49_gpu4.log"
touch "$MARK/gpu4.stage49.done"

echo "[$(date '+%F %T')] queue_sdpi_gpu4 complete"
