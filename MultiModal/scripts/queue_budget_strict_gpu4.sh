...
python -m MultiModal.multimodal.experiments.run_stage13_federated_comparison \
  --config MultiModal/configs/stage13_budget_strict_gpu4.yaml 2>&1 | tee "$LOG_DIR/stage13_gpu4.log"
...
python -m MultiModal.multimodal.experiments.run_stage16_budget_matched_frontier \
  --config MultiModal/configs/stage16_budget_strict_gpu4.yaml 2>&1 | tee "$LOG_DIR/stage16_gpu4.log"

echo "[$(date '+%F %T')] strict gpu4 queue complete."
