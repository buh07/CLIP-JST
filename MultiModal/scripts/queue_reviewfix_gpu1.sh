...
rm -f "$OUT/split/gpu1/markers/stage41_semantic_metadata_topk.done.json"
python -m MultiModal.multimodal.experiments.run_stage41_semantic_metadata_topk \
  --config MultiModal/configs/stage41_semantic_metadata_gpu1.yaml 2>&1 | tee "$LOG_DIR/stage41_gpu1.log"

touch "$MARK/gpu1.stage41.done"
echo "[$(date '+%F %T')] queue_reviewfix_gpu1 complete"
