...
for attempt in 1 2 3; do
  if python -m MultiModal.multimodal.experiments.run_stage69_third_triple_speechcoco --config MultiModal/configs/stage69_speechcoco_gpu4.yaml 2>&1 | tee "$LOG_DIR/stage69_gpu4.attempt${attempt}.log"; then
    touch "$MARK/gpu4.stage69.done"
    echo "[$(date '+%F %T')] queue_stage69_gpu4 complete"
    exit 0
  fi
  echo "[$(date '+%F %T')] queue_stage69_gpu4 attempt $attempt failed"
  if [[ "$attempt" -lt 3 ]]; then sleep 30; fi
done

touch "$MARK/gpu4.stage69.failed"
exit 1
