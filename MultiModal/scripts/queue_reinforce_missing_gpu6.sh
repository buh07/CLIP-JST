...
# W2
run_or_skip   "$MM_ROOT/configs/stage20_w2_joint_clap_gpu6.yaml"   MultiModal.multimodal.experiments.run_stage20_modular_audio_transitivity   "$LOG_DIR/w2_joint_clap.gpu6.log"
touch "$MARK_DIR/w2_gpu6.done"
wait_markers w2 8
if [[ "6" == "7" ]]; then
...
# W9
run_or_skip   "$MM_ROOT/configs/stage25_w9_replication_gpu6.yaml"   MultiModal.multimodal.experiments.run_stage25_modtrans_jl_ablation   "$LOG_DIR/w9_replication.gpu6.log"
touch "$MARK_DIR/w9_gpu6.done"
wait_markers w9 8
if [[ "6" == "7" ]]; then
...
# W11
run_or_skip   "$MM_ROOT/configs/stage61_w11_shuffled_gpu6.yaml"   MultiModal.multimodal.experiments.run_stage61_shuffled_caption_control   "$LOG_DIR/w11_shuffled.gpu6.log"
touch "$MARK_DIR/w11_gpu6.done"
wait_markers w11 8
...
run_or_skip   "$MM_ROOT/configs/stage62_w3_clotho_gpu6.yaml"   MultiModal.multimodal.experiments.run_stage62_clotho_intermediate_eval   "$LOG_DIR/w3_clotho_eval.gpu6.log"
touch "$MARK_DIR/w3_gpu6.done"
wait_markers w3 8
if [[ "6" == "7" ]]; then
...
touch "$MARK_DIR/reinforce_missing_gpu6.all_done"
echo "[$(date '+%F %T')] queue_reinforce_missing_gpu6 complete"
