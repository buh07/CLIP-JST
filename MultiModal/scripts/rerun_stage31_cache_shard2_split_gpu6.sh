...
python MultiModal/scripts/build_wavcaps_cache_shard.py \
  --shard-out-dir "$SHARD_ROOT/shard2_split_a" \
  ...
  --shard-index 2 \
  --shard-count 16 \
  --done-marker "$MARK_DIR/wavcaps_cache.shard2a.done.json" \
  --fail-marker "$MARK_DIR/wavcaps_cache.shard2a.fail.json" \
  2>&1 | tee "$LOG_DIR/stage31_cache_shard2a_gpu6.log"
