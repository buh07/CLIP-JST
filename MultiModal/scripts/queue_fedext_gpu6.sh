...
python -m MultiModal.multimodal.experiments.run_stage13_federated_comparison \
  --config MultiModal/configs/stage13_federated_gpu6.yaml 2>&1 | tee "$LOG_DIR/stage13_gpu6.log"
...
python -m MultiModal.multimodal.experiments.run_stage14_stronger_attacks \
  --config MultiModal/configs/stage14_stronger_attacks_gpu6.yaml 2>&1 | tee "$LOG_DIR/stage14_gpu6.log"
python - <<'PY'
from pathlib import Path
from MultiModal.multimodal.common import mark_done
mark_done(Path('/jumbo/lisp/f004ndc/CLIP JST/MultiModal/results/federated_extension/markers/stage14_gpu6.done.json'))
PY
