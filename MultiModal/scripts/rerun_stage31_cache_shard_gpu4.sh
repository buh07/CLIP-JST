...
python MultiModal/scripts/build_wavcaps_cache_shard.py \
  --shard-out-dir "$SHARD_ROOT/shard0" \
  ...
  --shard-index 0 \
  --shard-count 4 \
  --done-marker "$MARK_DIR/wavcaps_cache.shard0.done.json" \
  --fail-marker "$MARK_DIR/wavcaps_cache.shard0.fail.json" \
  2>&1 | tee "$LOG_DIR/stage31_cache_shard_gpu4.log"
