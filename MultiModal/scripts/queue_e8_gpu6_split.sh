...
run_step "stage5_e8a_gpu6_coco_s0" "python -m MultiModal.multimodal.experiments.run_stage5_e8a_concat --config MultiModal/configs/stage5_e8a_gpu6_coco_s0.yaml"

wait_eval_count "$OUT_ROOT/stage5_e8a/coco" 108 "stage5_coco"
wait_eval_count "$OUT_ROOT/stage5_e8a/flickr30k" 108 "stage5_flickr30k"
...
echo "[$(date '+%F %T')] Gate decision: run_stage6=$RUN_STAGE6 run_stage7=$RUN_STAGE7 run_stage8=$RUN_STAGE8"

if [[ "$RUN_STAGE6" == "true" ]]; then
  run_step "stage6_e8b_gpu6_coco_s0" "python -m MultiModal.multimodal.experiments.run_stage6_e8b_mask_concat --config MultiModal/configs/stage6_e8b_gpu6_coco_s0.yaml"
...
if [[ "$RUN_STAGE8" == "true" ]]; then
  wait_file "$MARKERS/stage8_e8d_eps14.done.json"
  wait_file "$MARKERS/stage8_e8d_eps8.done.json"
  run_step "stage8_e8d_merge" "python -m MultiModal.multimodal.experiments.run_stage8_e8d_merge --config MultiModal/configs/stage8_e8d_merge.yaml"
fi

if [[ "$RUN_STAGE7" == "true" ]]; then
  wait_file "$MARKERS/stage7_e8c.done.json"
fi
if [[ "$RUN_STAGE8" == "true" ]]; then
  wait_file "$MARKERS/stage8_e8d.done.json"
fi
...
echo "[$(date '+%F %T')] GPU6 split coordinator queue complete."
