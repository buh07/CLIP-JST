...
python -m MultiModal.multimodal.experiments.run_stage2_e7_karpathy \
  --config MultiModal/configs/stage2_e7_strength_gpu4_coco.yaml 2>&1 | tee "$LOG_DIR/stage2_strength_gpu4.log"

python -m MultiModal.multimodal.experiments.run_stage13_federated_comparison \
  --config MultiModal/configs/stage13_fedfix_gpu4.yaml 2>&1 | tee "$LOG_DIR/stage13_fedfix_gpu4.log"
...
python -m MultiModal.multimodal.experiments.run_stage14_stronger_attacks \
  --config MultiModal/configs/stage14_fedfix_gpu4.yaml 2>&1 | tee "$LOG_DIR/stage14_fedfix_gpu4.log"

echo "[$(date '+%F %T')] queue_strength_gpu4 complete"
