"""W5-S44: Gap_ia → α regression using COCO Phase A data.

Loads per-method, per-dim, per-seed (gap_ia_l2, av_ia_avg_R) from Stage 39 S44
(COCO Phase A checkpoints) and per-method α from Stage 36 S44 (COCO Phase A refit).
Runs OLS linear regression of gap_ia_l2 → α across methods and dims (pooled seeds),
and saves results to results/reviewer_fixes_suite/w5_s44_regression/.
"""
...
_STAGE36_PATH = (
    _MM_ROOT
    / "results/reviewer_fixes_suite/stage36_s44_coco"
    / "stage36_bottleneck_decomposition"
    / "stage36_bottleneck_decomposition.json"
)

_STAGE39_PATH = (
    _MM_ROOT
    / "results/reviewer_fixes_suite/stage39_s44_coco"
    / "stage39_modality_gap_linear_vs_jl"
    / "stage39_modality_gap_linear_vs_jl_results.json"
)

_OUTPUT_DIR = (
    _MM_ROOT
    / "results/reviewer_fixes_suite/w5_s44_regression"
)
...
def run() -> None:
    ...
    # Per-method α from Stage 36 S44
    method_alpha: dict[str, float] = {
        method: float(vals["alpha"])
        for method, vals in stage36["by_method"].items()
    }
    print("Stage 36 S44 per-method α:", method_alpha)

    # Build flat table from Stage 39 S44
    raw39 = stage39["raw"]
    ...
    print(f"Collected {len(rows)} (method, dim, seed) rows from Stage 39 S44")
    ...
    print(f"\nPooled gap_ia → α (n={len(rows)}):  r={pooled_gap_to_alpha['pearson_r']:.4f}")
    print(f"Pooled gap_ia → av_ia (n={len(rows)}): r={pooled_gap_to_avR['pearson_r']:.4f}")
    print(f"Pooled α → av_ia (n={len(rows)}):      r={pooled_alpha_to_avR['pearson_r']:.4f}")

    # Per-dim: gap_ia → α across methods
    ...
    for dim_key in raw39:
        ...
        print(f"  m={dim}: gap→α r={reg_ga['pearson_r']:.4f}, gap→av_ia r={reg_gavR['pearson_r']:.4f}")

    # Per-method: gap_ia → av_ia across dims+seeds
    ...
    for method in method_alpha:
        ...
        print(f"\n  {method}: α={method_alpha[method]:.4f}, gap_mean={np.mean(g):.4f}, av_ia_mean={np.mean(avR):.4f}")

    result = {
        "stage": "w5_s44_regression",
        "description": "OLS regression: gap_ia_l2 → α and → av_ia_avg_R; Stage36 S44 + Stage39 S44 (COCO Phase A)",
        "stage36_source": str(_STAGE36_PATH),
        "stage39_source": str(_STAGE39_PATH),
    ...
    out_path = _OUTPUT_DIR / "w5_s44_regression_results.json"
