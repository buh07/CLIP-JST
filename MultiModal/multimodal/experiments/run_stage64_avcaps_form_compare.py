...
def _merge_stage57_raw(stage57_blocks: list[dict[str, Any]]) -> dict[str, dict[str, list[dict[str, Any]]]]:
...
def _collect_records(stage58_aggregate_path: Path) -> tuple[list[dict[str, Any]], list[str]]:
    stage58 = load_json(stage58_aggregate_path)
    roots = [Path(x).resolve() for x in stage58["stage57_shard_roots"]]
    blocks = []
    for r in roots:
        p = r / "stage57_second_triple_avcaps" / "stage57_second_triple_avcaps_results.json"
        if not p.exists():
            raise FileNotFoundError(f"Missing stage57 shard results: {p}")
        blocks.append(load_json(p))
    raw = _merge_stage57_raw(blocks)
    recs = []
...
def run(cfg: dict[str, Any]) -> None:
    ...
    stage_root = out_root / "stage64_avcaps_form_compare"
    stage_root.mkdir(parents=True, exist_ok=True)
    markers = out_root / "markers"
    markers.mkdir(parents=True, exist_ok=True)

    stage58_path = Path(cfg["stage58_aggregate"]).resolve()
    records, roots = _collect_records(stage58_path)
    ...
    out = {
        "stage": "stage64_avcaps_form_compare",
        "generated_at": time.strftime("%Y-%m-%d %H:%M:%S"),
        "stage58_aggregate": str(stage58_path),
        "stage57_shard_roots": roots,
        "analysis": analysis,
        "elapsed_sec": float(time.time() - start),
    }
    save_json(out, stage_root / "stage64_avcaps_form_compare.json")

    lines = [
        "# Stage64 AVCaps Functional-Form Comparison",
        "",
        f"- n_total: {analysis['n_total']}",
        "",
        "| Form | In-sample R² | In-sample MAE | 5-fold CV R² (mean±std) | 5-fold CV MAE (mean±std) | LOMO R² | LOMO MAE | LODO R² | LODO MAE |",
        "|---|---:|---:|---:|---:|---:|---:|---:|---:|",
    ]
    for m in analysis["models"]:
        lines.append(
            ...
            f"{m['lomo_r2']:.4f} | {m['lomo_mae']:.5f} | {m['lodo_r2']:.4f} | {m['lodo_mae']:.5f} |"
        )
    ci = analysis["power_law_free_ci95"]
    lines += [
        "",
        f"- Free power-law bootstrap CI95: a ∈ [{ci['a_lo']:.3f}, {ci['a_hi']:.3f}], b ∈ [{ci['b_lo']:.3f}, {ci['b_hi']:.3f}]",
    ]
    (stage_root / "stage64_avcaps_form_compare.md").write_text("\n".join(lines), encoding="utf-8")
    ...
    save_json(provenance, stage_root / "provenance_stage64.json")
    mark_done(markers / "stage64_avcaps_form_compare.done.json", {"elapsed_sec": float(time.time() - start)})
    print("stage64_avcaps_form_compare complete")
